...
extern ARM_DRIVER_USART ARM_Driver_USART_(PUMP_TL);
...
/**
 * @function    void myUART_callback_pump_tl(uint32_t event)
 * @brief       UART isr callabck
 * @note        none
 * @param       event: USART Event
 * @retval      none
 */
void myUART_callback_pump_tl(uint32_t event)
{
    if (event & ARM_USART_EVENT_SEND_COMPLETE)
    {
        /* Send Success */
    ...
    {
        /* Receive Success */
        event_flags_uart_pump_tl |= UART_CB_RX_EVENT;
    }
